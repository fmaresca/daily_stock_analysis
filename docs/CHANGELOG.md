...
## [Unreleased]
- [新功能] 期权交易质量模拟器未收录标的实时财报日程自动检索与网络延迟通知引擎 (Options Trade Quality Simulator Automated Live Earnings Calendar Retrieval & Latency Notification Engine): (1) 针对本地未预置财报日程的自选股或输入标的，在 web/src/utils/earningsCalendar.ts 中交付 isStoredInEarningsRegistry 与 fetchLiveEarningsInfo 自动查询引擎，自动暂停并跨代理并发查询公开披露与财报日历接口 (Yahoo Finance quoteSummary calendarEvents, v7 quote timestamps, 财报季节周期推演)；(2) 在 OptionsTradeQualitySimulator.tsx 中实现自动化侦测与挂起拉取，通过醒目状态通知条 (⏳ Pausing to fetch corporate earnings calendar for {ticker}...) 与阶段进度状态条向用户提供清晰的延时与执行反馈；(3) 成功获取后自动将财报日程、盘前/盘后时段与财报季度持久化写入浏览器 localStorage 缓存 (deltaharvest_earnings_cache) 与内存注册表，并毫秒级触发平值跨式期权隐含波动 (ATM Straddle Implied Move) 与防御行权价实时重算；(4) 同步更新使用手册 (HelpHandbookModal.tsx) 与选股文档 (docs/automated-options-screener.md)。
