...
## [Unreleased]
- [修复] 部署版本滚动分包加载容错与动态导入自愈引擎 (Dynamic Import Chunk Load Recovery & Stale Cache Deployment Self-Healing Engine): (1) 交付 lazyWithRetry (web/src/utils/lazyWithRetry.ts) 弹性动态加载装饰器，捕获因 Cloudflare Pages 新版本发布导致客户端旧 chunk 哈希失效的 Failed to fetch dynamically imported module 异常，自动执行单次透明热刷新同步最新资源清单，彻底消除跨版本切换或部署瞬间用户视图报错；(2) 在 App.tsx 与 AppModalsContainer.tsx 中将全部代码分割视图与模态框升级为 lazyWithRetry；(3) 在 main.tsx 中注册全局 vite:preloadError 监听器并在 ErrorBoundary 中增设分包失效自愈与高光更新重载引导；(4) 在 web/public/_headers 中对根路由 /、/login 及 /dashboard 显式配置 Cache-Control: public, max-age=0, must-revalidate 阻断旧版 HTML 浏览器死锁缓存。
